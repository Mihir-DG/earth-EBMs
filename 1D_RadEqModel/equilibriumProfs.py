...
np.save("../pT/RE_pT",potentialTemperatures)
inter = np.array(interface_airPressure_vertCoord)
np.save("../pT/airP",inter)
...
ax.plot((np.array(sw_heatRate).flatten() + np.array(lw_heatRate).flatten()),
	interface_airPressure_vertCoord, '-o', color='green', label = 'Net')
